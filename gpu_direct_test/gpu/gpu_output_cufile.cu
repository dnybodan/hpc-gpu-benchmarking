...
//     FILE *log_file = fopen("throughput_cufile.log", "w");
//     if (log_file == NULL) {
...
//     for (int i = 0; i < 100; ++i) { // Run a sufficient number of iterations
...
int main() {
    ...
    // Initialize GPU
    cudaMalloc((void **)&d_data, BUFFER_SIZE);
    generateDataKernel<<<(BUFFER_SIZE + 255) / 256, 256, 0, stream>>>(d_data, BUFFER_SIZE);

    // Synchronize the stream to ensure data generation is complete
